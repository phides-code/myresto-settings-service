...
-   Find and replace `Myresto` with the name of the app (upper and lowercase A)
-   Find and replace `Setting`/`Settings` with the table name (upper and lowercase B)
...
-   `curl -X POST -H "Content-Type: application/json" http://localhost:8000/settings -d @post-data.json |jq .`
